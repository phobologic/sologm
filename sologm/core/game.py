...
class GameManager(BaseManager[Game, Game]):
    ...
    def create_game(self, name: str, description: str, is_active: bool = True) -> Game:
        ...
        def _create_game(session: Session, name: str, description: str, is_active: bool) -> Game:
    ...
    def _handle_integrity_error(self, error: IntegrityError, operation: str, name: str) -> None:
        ...
        if "unique constraint" in error_msg:
            if "name" in error_msg:
                raise GameError(f"A game with the name '{name}' already exists") from error
            elif "slug" in error_msg:
                raise GameError(f"A game with a similar name '{name}' already exists") from error
            else:
                raise GameError(f"Could not {operation} game due to a uniqueness constraint") from error
        else:
    ...
    def get_game_by_id(self, game_id: str) -> Optional[Game]:
        ...
        try:
            return self._execute_db_operation("get game by id", _get_game_by_id, game_id)
        except Exception as e:
    ...
    def get_game_by_slug(self, slug: str) -> Optional[Game]:
        ...
        try:
            return self._execute_db_operation("get game by slug", _get_game_by_slug, slug)
        except Exception as e:
    ...
    def activate_game(self, game_id: str, deactivate_others: bool = True) -> Game:
        ...
        def _activate_game(session: Session, game_id: str, deactivate_others: bool) -> Game:
            game = session.query(Game).filter(Game.id == game_id).first()
            if not game:
        ...
        try:
            return self._execute_db_operation("activate game", _activate_game, game_id, deactivate_others)
        except Exception as e:
    ...
    def deactivate_game(self, game_id: str) -> Game:
        ...
        try:
            return self._execute_db_operation("deactivate game", _deactivate_game, game_id)
        except Exception as e:
            logger.error(f"Failed to deactivate game {game_id}: {str(e)}")
            raise GameError(f"Failed to deactivate game {game_id}: {str(e)}") from e

    def update_game(self, game_id: str, name: Optional[str] = None, description: Optional[str] = None) -> Game:
        ...
        def _update_game(
            session: Session, game_id: str, name: Optional[str], description: Optional[str]
        ) -> Game:
            game = session.query(Game).filter(Game.id == game_id).first()
            if not game:
