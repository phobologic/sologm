    ...
    if literal_block_fields:
        ...
        for field in literal_block_fields:
            if field in processed_data and processed_data[field]:
                # Ensure the field uses literal block style
                if not processed_data[field].endswith('\n'):
    ...
    while retry_count <= max_retries:
        ...
        if current_error:
            ...
            for line in current_text.split('\n'):
                if line.strip().startswith('#'):
                    ...
                    if in_original_block and line.strip() == "#":
                        ...
                        continue
                    if not in_original_block or "Original values (for reference):" in line:
                        header_lines.append(line)
            ...
            for line in error_header.split('\n'):
                error_header_lines.append(f"# {line}" if not line.startswith('#') and line.strip() else line)
        ...
        if not was_modified:
            if retry_count > 0:
                console.print("[yellow]No additional changes made. Canceling edit.[/yellow]")
            return data, False
        ...
        try:
            ...
            if required_fields:
                for field in required_fields:
                    if field not in edited_data or not str(edited_data.get(field, "")).strip():
            ...
            if retry_count < max_retries:
                ...
                console.print("[bold red]Maximum retry attempts reached. Canceling edit.[/bold red]")
                return data, False
    
    # If we've exhausted retries
    console.print("[bold red]Failed to parse YAML after multiple attempts. No changes made.[/bold red]")
    return data, False
