...
class Child(Base):
    ...
    def __repr__(self):
        return (
            f"<Child(id={self.id}, name='{self.name}', "
            f"parent_id={self.parent_id}, obj_id={id(self)})>"
        )
...
class SimpleManager:
    ...
    def _execute_db_operation_problematic(
        ...
        try:
            ...
            self.logger.debug(
                f"[Problematic] Operation {operation_name} successful, "
                f"calling commit() on session ID: {id(session)}"
            )
            ...
            self.logger.error(
                f"[Problematic] Error in {operation_name}, rolling back "
                f"session ID: {id(session)}",
                exc_info=True,
            )
            session.rollback()
            raise
    ...
    def _execute_db_operation_fixed(
        ...
        try:
            ...
            self.logger.error(
                f"[Fixed] Error in {operation_name}, rolling back "
                f"session ID: {id(session)}",
                exc_info=True,
            )
            session.rollback()
    ...
    def create_parent_problematic(self, name: str) -> Parent:
        def _create(session: Session):
            ...
            # Note: The problematic commit happens *after* this returns
            # in _execute_db_operation_problematic
            session.flush()
            self.logger.debug(
                f"[Problematic] Flushed session {id(session)}, "
                f"Parent has ID: {parent.id}"
            )
            return parent
    ...
    def add_child_problematic(self, parent: Parent, child_name: str) -> Child:
        def _add(session: Session):
            # Important: Use the parent object passed in,
            # which might be from the test's session context
            self.logger.debug(
                f"[Problematic] Adding child '{child_name}' to Parent: {parent}"
            )
            # We might need to ensure the parent is in *this* session state
            # if it came from elsewhere
            # For simplicity, assume parent is already associated or merge
            # handles it (though merge is tricky)
            # parent_in_session = session.merge(parent)
            # Using merge can add its own complexities
            # self.logger.debug(
            #     f"[Problematic] Parent merged into session "
            #     f"{id(session)}: {parent_in_session}"
            # )
        ...
        return self._execute_db_operation_problematic("add_child", _add)
    ...
    def add_child_fixed(self, parent: Parent, child_name: str) -> Child:
        def _add(session: Session):
            ...
            # self.logger.debug(
            #     f"[Fixed] Parent merged into session "
            #     f"{id(session)}: {parent_in_session}"
            # )
        ...
        return self._execute_db_operation_fixed("add_child", _add)
...
def test_session(test_engine):
    ...
    try:
        yield session
        logger.debug(
            f"[Fixture test_session] Test finished, committing session "
            f"ID: {id(session)}"
        )
        session.commit()  # Commit everything at the end of the test
    except Exception:
        logger.error(
            f"[Fixture test_session] Exception during test, rolling back "
            f"session ID: {id(session)}"
        )
        session.rollback()
        raise
...
def problematic_manager(test_session):
    ...
    logger.debug(
        f"[Fixture problematic_manager] Creating manager with session "
        f"ID: {id(test_session)}"
    )
...
def test_failure_with_intermediate_commit(
    ...
    logger.debug(
        f"Test using problematic_manager "
        f"(obj_id={id(problematic_manager)}) with session ID: "
        f"{id(test_session)}"
    )
    ...
    # Note: The object IDs might be the same or different depending on
    # session state after commit/expiry

    # 4. Attempt to access the relationship (lazy load)
    logger.debug(
        f"Test attempting to access fetched_parent.children on session "
        f"ID: {id(test_session)}"
    )
    ...
    logger.error(
        f"Successfully caught expected error: {exc_info.type.__name__} "
        f"- {exc_info.value}"
    )
    ...
    logger.info(
        "--- Finished test_failure_with_intermediate_commit "
...
def test_success_with_flush(fixed_manager: SimpleManager, test_session: Session):
    ...
    logger.debug(
        f"Test using fixed_manager (obj_id={id(fixed_manager)}) "
        f"with session ID: {id(test_session)}"
    )
