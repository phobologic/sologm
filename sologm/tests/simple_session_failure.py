...
class SimpleManager:
    def __init__(self, session: Optional[Session] = None):
        ...
        self.logger.debug(f"Initialized SimpleManager with session ID: {id(self._session)}")
    ...
    def _get_session(self) -> Tuple[Session, bool]:
        ...
        if self._session is not None:
            ...
            raise NotImplementedError("Internal session creation not implemented for this example")

    def _execute_db_operation_problematic(self, operation_name: str, operation, *args, **kwargs):
        ...
        self.logger.debug(f"[Problematic] Got session ID: {id(session)}, was_provided={was_provided}")
        try:
            result = operation(session, *args, **kwargs)
            self.logger.debug(f"[Problematic] Operation {operation_name} successful, calling commit() on session ID: {id(session)}")
            session.commit() # <-- The problematic commit
            self.logger.debug(f"[Problematic] Commit finished for session ID: {id(session)}")
            return result
        except Exception as e:
            self.logger.error(f"[Problematic] Error in {operation_name}, rolling back session ID: {id(session)}", exc_info=True)
            session.rollback()
            raise
        finally:
    ...
    def _execute_db_operation_fixed(self, operation_name: str, operation, *args, **kwargs):
        """Mimics the FIXED BaseManager logic - FLUSHES provided sessions."""
        self.logger.debug(f"[Fixed] Executing: {operation_name}")
        session, was_provided = self._get_session()
        self.logger.debug(f"[Fixed] Got session ID: {id(session)}, was_provided={was_provided}")
        try:
            result = operation(session, *args, **kwargs)
            self.logger.debug(f"[Fixed] Operation {operation_name} successful")
            if not was_provided:
                 self.logger.debug(f"[Fixed] Committing internally created session ID: {id(session)}")
                 session.commit()
                 self.logger.debug(f"[Fixed] Commit finished for session ID: {id(session)}")
            else:
                 self.logger.debug(f"[Fixed] Flushing provided session ID: {id(session)}")
                 session.flush() # <-- Flush instead of commit for provided sessions
                 self.logger.debug(f"[Fixed] Flush finished for session ID: {id(session)}")
            return result
        except Exception as e:
            self.logger.error(f"[Fixed] Error in {operation_name}, rolling back session ID: {id(session)}", exc_info=True)
            session.rollback()
            raise
        finally:
    ...
    def create_parent_problematic(self, name: str) -> Parent:
        def _create(session: Session):
            ...
            session.flush()
            self.logger.debug(f"[Problematic] Flushed session {id(session)}, Parent has ID: {parent.id}")
            return parent
    ...
    def add_child_problematic(self, parent: Parent, child_name: str) -> Child:
        def _add(session: Session):
            ...
            self.logger.debug(f"[Problematic] Adding child '{child_name}' to Parent: {parent}")
            ...
            session.add(child)
            session.flush()
            self.logger.debug(f"[Problematic] Flushed session {id(session)}, Child has ID: {child.id}")
            return child
        return self._execute_db_operation_problematic("add_child", _add, parent, child_name)
    ...
    def create_parent_fixed(self, name: str) -> Parent:
        def _create(session: Session):
            ...
            self.logger.debug(f"[Fixed] Flushed session {id(session)}, Parent has ID: {parent.id}")
            return parent
        return self._execute_db_operation_fixed("create_parent", _create)

    def add_child_fixed(self, parent: Parent, child_name: str) -> Child:
        def _add(session: Session):
            self.logger.debug(f"[Fixed] Adding child '{child_name}' to Parent: {parent}")
            ...
            self.logger.debug(f"[Fixed] Flushed session {id(session)}, Child has ID: {child.id}")
            return child
...
def test_session(test_engine):
    ...
    try:
        yield session
        logger.debug(f"[Fixture test_session] Test finished, committing session ID: {id(session)}")
        session.commit() # Commit everything at the end of the test
    except Exception:
        logger.error(f"[Fixture test_session] Exception during test, rolling back session ID: {id(session)}")
        session.rollback()
        raise
    finally:
...
def problematic_manager(test_session):
    ...
    logger.debug(f"[Fixture problematic_manager] Creating manager with session ID: {id(test_session)}")
...
def fixed_manager(test_session):
    ...
    logger.debug(f"[Fixture fixed_manager] Creating manager with session ID: {id(test_session)}")
    return SimpleManager(session=test_session)


# --- Test Functions ---

def test_failure_with_intermediate_commit(problematic_manager: SimpleManager, test_session: Session):
    """
    ...
    logger.debug(f"Test using problematic_manager (obj_id={id(problematic_manager)}) with session ID: {id(test_session)}")
    ...
    logger.debug(f"Test fetching Parent with ID {parent_id} using session ID: {id(test_session)}")
    ...
    logger.debug(f"Test attempting to access fetched_parent.children on session ID: {id(test_session)}")
    ...
    with pytest.raises((DetachedInstanceError, Exception)) as exc_info: # Catch broader Exception initially if unsure
        children_count = len(fetched_parent.children)
        logger.debug(f"Accessed fetched_parent.children, count = {children_count}") # This line likely won't be reached

    logger.error(f"Successfully caught expected error: {exc_info.type.__name__} - {exc_info.value}")
    ...
    logger.info("--- Finished test_failure_with_intermediate_commit (Expected Failure Caught) ---")
...
def test_success_with_flush(fixed_manager: SimpleManager, test_session: Session):
    ...
    logger.debug(f"Test using fixed_manager (obj_id={id(fixed_manager)}) with session ID: {id(test_session)}")
    ...
    try:
        ...
        logger.error(f"Caught unexpected error: {type(e).__name__} - {e}", exc_info=True)
